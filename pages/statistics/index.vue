		...
		<view class="charts">
			<view class="title">
				...
				<u-subsection class="query-type" :list="queryTypeList" @change="queryTypeListChange" :animation="false"
					active-color="#42b479" :current="formData.queryType" height="60" font-size="25">
				</u-subsection>
			</view>
			<StatisticLeiXingBingTu :list="statisticTypeData">
			</StatisticLeiXingBingTu>
			<StatisticShouZhiZhuZhuangTu :list="statisticZhuData"> 
	...
	export default {
		...
		methods: {
			...
			getStatisticData() {
				...
				uni.request({
					url: 'https://10.133.140.131:7053/api/todo/getStatisticData',
					sslVerify: false,
					method: 'GET',
					data: params,
					header: {
					...
					success: (res) => {
						console.log("6")
						console.log(res)
						this.statisticTypeData = res.data.data;
						this.statisticZhuData=res.data.chartData
					},
					fail: (err) => {
						console.log(err);
					}
